...
import { submitQuestionService, startSSEStreamService, waitForReferencesReady } from "@/api/chat";
...
const handleSend = async (messageContent) => {
  try {
    ...
    // console.log(chatStore.getLastMessage());
    ...
    // 使用fetch 发送POST请求，并流式读取响应

    const post = { "text": messageContent.text, "information": messageContent.files }
    console.log(post)


    // 直接用 fetch 发 POST，拿响应流（SSE）
    const response = await fetch('http://localhost:8000/api/QAsystemStream', {
      method: "POST",
      ...
      for (const line of lines) {
        if (line.startsWith("data: ")) {
          ...
          try {
            ...
            chatStore.updateLastMessage(accumulatedContent, thinkContent, post.information);
          } catch (e) {
  ...
  <div class="chat-container">
    ...
    <div class="messages-container" ref="messagesContainer">
      <template v-if="currentMessages.length > 0">
        <chat-message v-for="(message, index) in currentMessages" :key="message.id" :message="message"
          :is-last-assistant-message="index === currentMessages.length - 1 && message.role === 'assistant'
