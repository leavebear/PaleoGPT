...
const uploadUrl = "http://localhost:8000/uploadFile"; // 上传文件的接口地址
...
const handleFileUpload = (uploadFile) => {
  ...
  // 避免重复添加
  let existingFile = fileList.value.find((f) => f.name === file.name && f.size === file.size);
  if (!existingFile) {
    existingFile = {
      name: file.name,
  ...
  reader.onload = (e) => {
    existingFile.content = e.target.result; // 读取内容后，直接更新已有的对象
  };
...
<template>
  <div class="chat-input-wrapper">
    ...
    <div class="button-group">
      ...
      >
        <el-tooltip content="支持上传附件" placement="top">
