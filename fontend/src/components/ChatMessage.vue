...
<template>
  <div class="message-item" :class="{ 'is-mine': message.role === 'user' }">
    <div class="content">
      ...
      <!-- 参考资料信息展开按钮 -->
      <div v-if="message.information" class="info-toggle" @click="toggleInfo">
        <span>查看参考资料</span>
        <el-icon class="toggle-icon" :class="{ 'is-expanded': isInfoExpanded }">
          <ArrowDown />
        </el-icon>
      </div>

      <!-- 参考资料内容 -->
      <div v-if="message.information && isInfoExpanded" class="information-box">
        <h3>参考资料</h3>
        <div class="info-items">
          <div
            class="info-item"
            v-for="item in message.information"
            :key="item.filename"
          >
            <div class="info-header">
              <span class="info-filename">{{ item.filename }}</span>
              <el-icon
                class="toggle-icon"
            ...
            <div v-if="item.isExpanded" class="info-content">
              <p>{{ item.content }}</p>
            </div>
          </div>
        </div>
